            ...
            <View style={styles.searchSectionWrap}>
              ...
              <View style={styles.matchingItemsList}>
                {matchingItems.map((item) => {
                  ...
                  return (
                    <View key={item._id} style={[styles.searchItemCard, isSingleMode && isSelected && { borderColor: COLORS.secondary, borderWidth: 2.5, backgroundColor: '#F7FEE7' }]}>
                      {/* Item Image / Icon */}
                      ...
                      <View style={styles.searchItemAction}>
                        {isSingleMode ? (
                          isSelected ? (
                            <TouchableOpacity
                              style={[styles.searchAddBtn, { backgroundColor: COLORS.secondary, paddingHorizontal: 8 }]}
                              activeOpacity={0.8}
                              onPress={() => handleAddToCart(item, -1)}
                            >
                              <Text style={[styles.searchAddText, { color: COLORS.black, marginLeft: 0, fontSize: 10 }]}>✓ SELECTED</Text>
                            </TouchableOpacity>
                          ) : (
                            <TouchableOpacity
                              style={[styles.searchAddBtn, { backgroundColor: COLORS.black, paddingHorizontal: 10 }]}
                              activeOpacity={0.8}
                              onPress={() => handleAddToCart(item, 1)}
                            >
                              <Text style={[styles.searchAddText, { color: COLORS.secondary, marginLeft: 0, fontSize: 10 }]}>1-CLICK</Text>
                            </TouchableOpacity>
                          )
                        ) : qty === 0 ? (
                          <TouchableOpacity
